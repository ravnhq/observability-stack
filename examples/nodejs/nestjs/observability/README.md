# LGTM Observability Stack for NestJS

This setup integrates the **LGTM Stack** (Loki, Grafana, Tempo, Mimir/Prometheus) with your NestJS application using OpenTelemetry.

## 🏗️ Architecture

```
┌─────────────────┐     ┌──────────────────────┐     ┌─────────────┐
│   NestJS App    │────▶│  OpenTelemetry       │────▶│   Tempo     │ (Traces)
│                 │     │  Collector           │     └─────────────┘
│  - Traces       │     │                      │     ┌─────────────┐
...
docker-compose up -d otel-collector grafana tempo loki prometheus postgres
...
### Dashboards (Grafana)
- Pre-configured NestJS Overview dashboard
...
| `OTEL_SERVICE_NAME` | Service name for telemetry       | `nestjs-app`               |
...
3. Search by service name: `nestjs-app`

### View Logs
1. Go to Grafana → Explore
2. Select "Loki" datasource
3. Query: `{service_name="nestjs-app"}`

### View Metrics
1. Go to Grafana → Explore
2. Select "Prometheus" datasource
3. Query: `http_server_request_duration_seconds_count{service_name="nestjs-app"}`
