...
log_info "Starting DELETE endpoint testing..."
...
while true; do
    ...
    # Make DELETE request
    ((TOTAL_REQUESTS++))
    response=$(http_request "DELETE" "$API_URL/users/$user_id")
    ...
    if [ "$status_code" = "204" ] || [ "$status_code" = "200" ]; then
        ...
        if [ "$status_code" = "404" ]; then
            log_warning "User not found (404): User may have been already deleted"
